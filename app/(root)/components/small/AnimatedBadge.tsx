...
export const AnimatedBadge = ({ text, icon, borderColor, className = "" }: AnimatedBadgeProps) => {
    return (
        <div
            className={`rounded-full p-[1px] h-12 bg-gradient-to-r from-transparent ${borderColor} to-transparent [background-size:400%_100%] ${className}`}
            style={{ animation: "move-bg 8s linear infinite" }}
        >
            <div className="inline-flex h-full items-center gap-2 rounded-full bg-white dark:bg-[#0a091e] px-4 py-1.5 text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-transparent">
                {icon}
                <span>{text}</span>
            </div>
        </div>
    ...
    return (
        ...
        `}
            ...
            <div
                ...
                onClick={onClick}
                className="relative  z-10 h-12 w-fit overflow-hidden font-sans antialiased bg-white dark:bg-black"
            >
                <AnimatedBadge text={title} icon={icon || <FlowerIcon />} borderColor={"via-indigo-500"} />
                {/* {animatedGradientColors.map((colorClass, index) => (
                            <AnimatedBadge key={index} text="Sera UI" icon={<FlowerIcon />} borderColor={colorClass} />
                        ))} */}
            </div>
        </>
    );
